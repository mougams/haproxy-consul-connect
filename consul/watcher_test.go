...
func TestWatcherConfigInit(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, _, err := consul.PreparedQuery().Create(&api.PreparedQueryDefinition{
				Name: "pq-",
...
func TestWatcherConfigUpdate(t *testing.T) {
	sd := lib.NewShutdown()
	defer sd.Shutdown("test end")
	...
	w := New("client-inst", consul, log.New())
